...
-- oracle에서는 nullif
-- mysql은 ifnull
select empno, comm, nullif(null,0) from emp;
...
select empno, ename from emp;
select empn
o, ename from emp where ename='SMITH';
...
select job from emp where job=lower('MANAGER');
...
select hiredate from emp;
select ename, deptno from emp where hiredate = TO_date('1981/09/28','YYYY-MM-DD');
select ename, deptno from emp where hiredate = TO_date('1981-09-28', 'YYYY-MM-DD');
--select ename, deptno from emp where hiredate = TO_date('81-09-28','YY-MM-DD');

-- 27. 날짜 타입의 범위를 기준으로 검색
-- 범위비교시 연산자 : between~and 1980-12-17~1981-09-28
select TO_CHAR(hiredate,'YYYY-MM-DD') from emp where hiredate between TO_date('1980-12-17','YYYY-MM-DD') and TO_DATE('1981-09-28','YYYY_MM_DD');
